...
# Issue #129: 標準ウォッチの承認境界

2026-09-23: 親#128のD1〜D11と#129本文に従い、1社/最大5監視元/OWNER1名の
標準サービスを実装する。旧実行日帳票を維持し、公開期間とimmutable納品版を
別契約として追加。候補選別を維持した全文比較を永続run/有限分割で行い、
OWNER認証・非公開再取得・終了後90日保持を必須とする。既存Manual Jobと
Azure環境を再利用。旧試験予算を移転しない。#66の顧客別権限は後続のまま。
実装・配備と本番受入/PC_OFFは別判定。最終GOまでは#129/#128を閉じない。

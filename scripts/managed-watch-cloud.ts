...
import { parseManagedCloudConfiguration } from "../src/lib/patent-watch/managed-cloud-config";
...
if(require.main===module){
  ...
  const watchdog=setTimeout(stop,95*60_000);process.on("SIGTERM",stop);process.on("SIGINT",stop);
  void(async()=>{
    ...
    try{
      ...
      const config=parseManagedCloudConfiguration(JSON.parse(process.env.MANAGED_WATCH_CONFIG_JSON),Date.now(),false);
      ...
      try{
        for(const run of config.runs)await executeManagedRun(repository,run.caseId,run.runId,execution,managedAzureAnalysis,{operationId:config.operationId,snapshotDigest:run.snapshotDigest});
    ...
    finally{await connection?.client.end().catch(()=>undefined);clearTimeout(watchdog);process.removeListener("SIGTERM",stop);process.removeListener("SIGINT",stop);}
  })();
}

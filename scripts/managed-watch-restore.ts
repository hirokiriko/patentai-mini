...
import { MANAGED_BACKUP_REFERENCES, parseManagedCaseBackup } from "../src/repositories/managed-backup";
...
export async function verifyManagedBackupRestore(bytes: Buffer, sha256: string, caseId: number, backupId: string) {
  ...
  const environment = await isolatedPg16(129);
  try {
    ...
    for (let i = 0; i < backup.artifacts.length; i++) {
      ...
      await writeFile(file, Buffer.from(artifact.data, "base64"), { flag: "wx" });
      const readBack = await readFile(file); archiveCheck(readBack.length === artifact.bytes && archiveSha(readBack) === artifact.sha256);
    }

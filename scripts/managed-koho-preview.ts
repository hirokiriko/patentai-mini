...
const input = z.object({ schema: z.literal(1), sourcePath: z.string().min(1).max(4096).refine(isAbsolute),
  byteLength: z.number().int().positive().max(2 * 1024**3), sha256: z.string().regex(/^[a-f0-9]{64}$/) }).strict();
export async function previewManagedKoho(value: unknown) {
  ...
  const parsed = await parseKohoPackage({ packageType: "JPA", source: { type: "file", path: config.sourcePath }, limits: buildKohoManualImportLimits(config.byteLength) });

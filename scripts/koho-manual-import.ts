...
async function superviseFile(input: WorkerInput, deadline: number, signal?: AbortSignal) {
  ...
  await new Promise<void>(resolvePromise => {
    ...
    const finish = () => { finished = true; clearTimeout(timer); clearTimeout(killTimer);
    ...
    try {
      ...
      child.on("message", (event: WorkerEvent) => {
        if (event.type === "progress" || event.type === "result") result = event.result;
      });
...
export async function runManualBatch(value: unknown, options: { deadlineMs?: number; signal?: AbortSignal } = {}) {
  ...
  let cleanup = "complete", stopped = false;
  const sizes: number[] = [];
  // Admit the whole list before any write. Copying rechecks these byte bounds.
  try {
    let total = 0;
  ...
  for (let index = 0; index < config.files.length && !stopped; index++) {
    ...
    try {
      ...
      const child = await superviseFile({ config, index, size: sizes[index], directory }, deadline, options.signal);
  ...
  return { status: unknown ? "reconciliation_required" : stopped ? "stopped" : "complete", mode: config.mode,
    results, savedRecordCount: results.filter(r => r.outcome === "inserted").reduce((n, r) => n + r.savedDocumentCount, 0),
  ...
  void (async () => {
    ...
    try {
      ...
      if (worker) {
        ...
        const result = await runManualWorker(input as WorkerInput, emit);

...
function collectFallbackTerms(extracted: ExtractedClaims): string[] {
  const elementTerms = extracted.claims.flatMap((claim) =>
  ...
  return uniqueTerms([...elementTerms, ...textTerms]).slice(0, 8);
}

function taggedOrTerms(terms: string[], tag: "TX" | "CL"): string {
  const safeTerms = terms.slice(0, 4).map((term) => `${term}?/${tag}`);
  return safeTerms.length > 0 ? safeTerms.join("+") : `draft?/${tag}`;
}

function buildFallbackQueries(extracted: ExtractedClaims): SearchQuerySet {
  const terms = collectFallbackTerms(extracted);
  const core = terms.slice(0, 3);
  const synonyms = terms.slice(3, 6);
  const effects = extracted.effects.length > 0 ? extracted.effects : terms.slice(6, 8);
  const broadTerms = terms.slice(0, 4);
  const balancedTerms = core.length > 0 ? core : terms.slice(0, 2);
  const narrowTerms = balancedTerms.slice(0, 3);
  const baseTerm = terms[0] ?? extracted.title ?? "draft";
  ...
  return {
    ...
    broadQuery: `[${taggedOrTerms(broadTerms, "TX")}]`,
    balancedQuery: `[${taggedOrTerms(balancedTerms, "CL")}]`,
    narrowQuery: `[${narrowTerms.map((term) => `${term}/CL`).join("*") || "draft/CL"}]`,
    keywordQueries: [
      {
        theme: "Core terms",
        keywords: terms.slice(0, 10).join(" "),
      },
    ],
      ...
      spellingVariants: [
        {
          baseTerm,
          variants: [baseTerm],
          reason: "Fallback query generation kept the original term for manual review.",
          suggestedUse: "Use this as a starting point and add synonyms manually if needed.",
        },
      ],
      companyNameHints: [],
      additionalKeywordQueries: [
        {
          theme: "Manual broadening",
          keywords: terms.slice(0, 6).join(" "),
          note: "AI query generation was unavailable, so this deterministic candidate keeps the workflow moving.",
        },
      ],
      leakageRisks: [
        "AI query generation was temporarily unavailable. Review synonyms and spelling variants manually.",
      ],
    },
    excludedTerms: [],
    rationale: [
      "Generated by deterministic fallback because the AI query generation step was temporarily unavailable.",
    ],
  };
}
...
async function generateQueriesWithinBudget(
  ...
  try {
    ...
    console.warn(
      "[generate-queries] using fallback queries after AI failure"
    );
    object = buildFallbackQueries(extracted);
  }
